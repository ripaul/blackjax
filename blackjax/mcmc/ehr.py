...
#def setup_metric(metric_backend, max_cond=jnp.inf, min_det=0, max_det=jnp.inf):
    ...
    if metric_backend == 'chol':
        def build_metric(M):
            ...
            #L = lax.select(jnp.isnan(L).any(), jnp.linalg.cholesky(M+jnp.diag(diag_scale*jnp.diag(M))), L)

            #L = jnp.linalg.cholesky(M+jnp.diag(diag_scale*jnp.diag(M)))
            #L = jnp.linalg.cholesky(M+diag_scale*jnp.mean(jnp.diag(M))*jnp.eye(M.shape[0]))

            #L = lax.select(jnp.isnan(L).any(), jnp.linalg.cholesky(M), L)
            #L = lax.select(jnp.isnan(L).any(), jnp.linalg.cholesky(M+1e2), L)
            #L = lax.select(jnp.isnan(L).any(), jnp.linalg.cholesky(M+1e3), L)
